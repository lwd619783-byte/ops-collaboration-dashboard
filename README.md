...
Task 0.1 至 Task 1.3 已建立前端、数据库、统一身份与网页登录基线。Task 1.4 增加 `workspaces`、`workspace_members`、`workspace_invitations`，四级角色与成员 / 邀请状态机，默认拒绝 RLS、最小 RPC、Auth 原子预配置 trigger，以及受信任 `invite-workspace-member` Edge Function。前端提供工作空间门禁、成员目录、owner/admin 管理入口和首次受邀激活页；邀请邮箱在业务数据库只保留 SHA-256 摘要和遮罩提示。独立审计修复已完成数据库唯一 owner 强制、邀请 TTL 与过期重邀、**已有受邀身份的重发闭环与 lineage 保持**（`operation_kind` 区分首次 / 重发、`reissue_prepared` 状态与 `reissue_of_invitation_id` 关联、**统一确认边界 `confirm_workspace_auth_invitation_result`——Auth Admin 成功不等于业务邀请成功，每次 Auth 调用后都必须数据库确认**、锁后 `clock_timestamp()` 时间语义、failed reissue 可恢复且永不退回普通 prepared、`auth_user_conflict` 稳定冲突、**未确认 Auth 用户跨工作空间首次邀请被安全拒绝（不假报成功、不创建第二套身份）**、成员目录 `pending_invitation` 区分）、首次激活 `USER_UPDATED` 恢复、成员目录 profile 缺失回退、幂等并发加固与 Edge 真实入口 CI（`deno check`）。真实 Auth 集成验证（`npm run db:reissue:verify`，`pg` 已作为 dev dependency）打开 recovery 邀请邮件链接建立真实 Supabase 会话，已纳入 Database CI。详见 [工作空间与成员权限](docs/workspace-permissions.md)。正式完成仍以远端独立审计和 PR 合并为准。
