...
Task 3.2 在 Task 3.1 的项目任务、`can_read_task()` 和安全详情 deep link 基线上增加项目级只读任务中心。路由为 `/projects/:projectId/tasks`，形成“项目详情 → 任务中心 → 看板 / 列表 → 筛选 → 任务详情”的网页浏览闭环。本任务不提供任何任务状态或进度写入口。
...
Task 3.2 不实现 Task 3.3 状态流转 / 阻塞 / 取消 / 状态历史，不实现 Task 3.4 每日进展，不实现 Task 3.5 验收，不实现状态或进度 mutation、拖拽、批量操作、通知、飞书、微信小程序、私人任务、甘特图、团队负荷、全局“我的任务”或阶段 4 管理者工作台。
